...
-------------------------------------*/
size_t TextMeshLoader::allocate_cpu_data(
    const std::string& str,
...
-------------------------------------*/
size_t TextMeshLoader::allocate_gpu_data(const Atlas& atlas) noexcept
{
    std::vector<SceneMaterial>& materials = sceneData.materials;
    ...
    LS_LOG_GL_ERR();
    numBytes += (unsigned)totalMetaData.calc_total_vertex_bytes();

    ibo.set_data(totalMetaData.calc_total_index_bytes(), nullptr, buffer_access_t::VBO_STATIC_DRAW);
    LS_LOG_GL_ERR();
    numBytes += (unsigned)totalMetaData.calc_total_index_bytes();
...
) noexcept
{
    ...
    const size_t numBytes = allocate_cpu_data(str, vertexTypes, loadBounds);
