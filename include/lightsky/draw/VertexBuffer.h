...
/**
 * @brief This structure contains basic attribute data management for VBOs and
 * validation for VAOs.
 */
struct VBOAttrib {
    unsigned        count;          // Total number of these attributes.
    GLint           components;     // Number of "baseType" elements per vert.
    vertex_data_t   baseType;       // The generic makeup of each element per vert.
    GLboolean       normalized;     // is this fixed-point data that needs normalizing?
    GLsizei         stride;         // Stride between each vertex element
    const GLvoid*   offset;         // byte-offset to the first vertex in the buffer (NOT MEMORY ADDRESS).
};
